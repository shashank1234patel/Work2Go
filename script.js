...
// Sample initial static jobs list
const jobs = [
...
if ($('#jobList')) { 
  ['jobSearch','distanceFilter','salaryFilter','categoryFilter','modeFilter','ratingFilter','sortJobs'].forEach(id => $('#'+id).addEventListener('input',renderJobs)); 
  $('#clearFilters').addEventListener('click',()=>{document.querySelectorAll('.filters select').forEach(s=>s.selectedIndex=0);renderJobs()}); 
  renderJobs(); 
}

// Global Apply Click Handler
document.addEventListener('click', e => { 
  if(e.target.matches('.small-button.apply')) { 
...
// 2. Updated Job Form Submit Listener (Supabase Integration)
...
if (jobForm) {
  jobForm.addEventListener('submit', async (e) => {
    ...
    const jobData = {
      ...
      workers_required: $('#workersRequired')?.value || 1,
      contact_info: $('#contactInfo')?.value || '',
      deadline: $('#jobDeadline')?.value || null
    };

    // Save to Supabase 'give_jobs' table
    const { data, error } = await supabase.from('give_jobs').insert([jobData]);
...
if (location.pathname.endsWith('job-details.html')) {
  const job=jobs.find(j=>j.id===+new URLSearchParams(location.search).get('id'))||jobs[0]; 
  $('#detailTitle').textContent=job.title; 
  $('#detailEmployer').textContent=job.employer; 
  $('#detailRating').textContent=job.rating; 
  $('#detailPay').textContent=`₹${job.pay}/${job.payment}`; 
  $('#applyPay').textContent=`₹${job.pay}/${job.payment}`; 
  $('#detailHours').textContent=job.hours; 
  $('#detailDistance').textContent=job.distance?`${job.distance} km away`:'Online';
}